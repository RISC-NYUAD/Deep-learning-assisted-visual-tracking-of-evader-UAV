...
const double rate=0.2;         // learning rate - 0.2
const double psrThreshold=5.7;//8.2; // no detection, if PSR is smaller than this - 5.7
...
//	imshow("image_sub",image_sub);
	Mat bgr[2];   //destination array
	split(H,bgr);//split source  
//	imshow("H",bgr[1]);
	//imshow("IMAGE_SUB",IMAGE_SUB);
...
//	imshow("response",response);
	...
	// CHECK BRIGHTNESS
	//cout << "max BRIGHTNESS MOSSE value === " << maxVal << endl;
	//maxBright = maxVal;
	maxBrightMOSSE = maxVal;
	//END CHECK BRIGHTNESS
